...
import useProductionTracking, {
  type StatusFilter,
} from "../hooks/useProductionTracking";

/**
 * Every production order this user is entitled to see.
 *
 * NOBODY IN OMS RAISES THESE. SAP is the point of origin — a planner creates
 * the order in SAP B1 and the sync finds it — so `Production_Order` means
 * "watch", not "own". An approver additionally sees what is awaiting their
 * decision, flagged on the card; somebody holding both keys sees one merged
 * list. There is no mode to choose and no empty tab to explain.
 *
 * LAID OUT AS PAYMENT TRACKING AND BACKDATE, deliberately down to the
 * geometry: the same status-and-search header, the same gradient count bar
 * carrying Filter and the date control, the same card and the same
 * bottom-sheet filter. A person who has learnt one list has learnt all three,
 * and lists sitting side by side in one app should not each invent their own
 * furniture.
 */
export default function ProductionTrackingScreen() {
   ...
   * 2" opened a list of every month's pending orders and the count looked wrong
    ...
    (order: ProductionOrder) => {
      router.push({
        ...
        params: {
          ...
          actionable: actionable.has(order.id) ? "1" : "0",
  ...
  const openProgress = useCallback((order: ProductionOrder) => {
    router.push({
      pathname: "/(main)/production/tracking-progress",
      params: { id: String(order.id), from: "production/tracking" },
    } as never);
  }, []);
  ...
  const renderEmpty = () => {
    ...
    return (
      <View style={styles.emptyWrap}>
        ...
        />
        <Text style={styles.emptyTitle}>No production orders</Text>
        <Text style={styles.emptyHint}>
          {status || company || dateFilter || search
            ? "Try widening the filters above."
            : canApprove
              ? "Nothing is waiting for your approval."
              : "Synced orders will appear here."}
        </Text>
      </View>
    );
  };
...
/**
 * The FLOW's statuses, which are the ones the server filters on.
 *
 * "Completed" is deliberately absent, unlike BackDate: whether the decision
 * reached SAP lives in `flow.sap_status`, not in the flow status, and the API
 * takes no filter for it. Offering it here would silently return every
 * approved order and quietly mean something else.
 *
 * `Obsolete` is offered because it is a real outcome people go looking for —
...
const STATUS_OPTIONS: { label: string; value: StatusFilter }[] = [
  ...
  { label: "No longer planned", value: "OBSOLETE" },
];
